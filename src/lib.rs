...
/// Helper trait to convert from i32 literals into a target integer type.
...
/// A word in the CPU (at the moment this is a signed 32-bit integer).
pub type Word = i32;

bitflags! {
    /// These flags are generated by the instruction parsers containing
    /// a lot of useful flags for any given instruction.
    /// There are a number of overlaps, since only some flags are valid
    /// for any given instruction.
    flags InstructionFlags: u32 {
        /// Empty flag.
        const INST_NORMAL         = 0,

        /// This is a wide THUMB instruction (4-bytes vs 2-bytes).
        const INST_WIDE           = 1 << 0,

        /// This instruction will alter the condition flags in the CPSR.
        const INST_SET_FLAGS      = 1 << 1,
        /// This instruction operates on 2-byte memory locations.
        const INST_HALF           = 1 << 2,
        /// This instruction operates on single-byte memory locations.
        const INST_BYTE           = 1 << 3,
        /// This instruction operates on signed operands.
        const INST_SIGNED         = 1 << 4,
        /// This instruction only affects the top-half of the destination
        /// register.
        const INST_TOP            = 1 << 5,
        /// This instruction only affects the bottom-half of the destination
        /// register.
        const INST_BOTTOM         = 1 << 6,
        /// This register uses ```Align(PC,4)``` where PC is referenced.
        const INST_ALIGN          = 1 << 7,

        // LDR/STR
        /// This is the offset variant of the instruction.
        const INST_OFFSET         = 1 << 8,
        /// This is the post-index variant of the instruction.
        const INST_POSTINDEX      = 1 << 9,
        /// This is the pre-index variant of the instruction.
        const INST_PREINDEX       = 1 << 10,
        /// This instruction uses the unpriveleged memory protection table
        /// in protected mode.
        const INST_UNPRIV         = 1 << 11,

        // LDM/STM
        /// This instruction uses memory acquire semantics.
        const INST_ACQUIRE        = 1 << 12,
        /// This instruction writes back to the source register.
        const INST_WRITEBACK      = 1 << 13,
        /// This instruction decrements (rather than increments) the source register.
        const INST_DECREMENT      = 1 << 14,
        /// This instruction alters the source register before reading
        /// (rather than altering it afterwards).
        const INST_BEFORE         = 1 << 15,
        /// This instruction is actual PUSH/POP.
        const INST_PUSHPOP        = 1 << 16,

        // B
        /// This is a linked branch (the current PC is stored in LR).
        const INST_LINK           = 1 << 12,
        /// This is an exchange branch (it updates the THUMB bit in the CPSR).
        const INST_EXCHANGE       = 1 << 13,
        /// This is a Jazelle branch and enters Jazelle mode.
        const INST_JAZELLE        = 1 << 14,
        /// This instruction is the not-zero variant (for example CBNZ, vs CBZ).
        const INST_NONZERO        = 1 << 15,

        // PKH
        /// This is the TB-form of the instruction.
        const INST_TBFORM         = 1 << 11,

        // STC
        /// This is the alternate version of the instruction (for example STC2 vs STC).
        const INST_ALT            = 1 << 11,
        /// This is the pre-increment variant of the instruction.
        const INST_PREINC         = 1 << 12,
        /// This instruction causes a long memory load.
        ...
        /// The PSTATE.A field should be affected.
        const INST_PSTATEA        = 1 << 4,
        /// The PSTATE.I field should be affected.
        const INST_PSTATEI        = 1 << 5,
        /// The PSTATE.F field should be affected.
...
/// An enum which can hold either a register or an immediate value of
/// type `T`.
